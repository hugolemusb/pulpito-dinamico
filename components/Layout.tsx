...
const AI_PRESETS: Record<string, { label: string; provider: AIProvider; baseUrl: string; modelId: string; icon: any; apiKey?: string }> = {
  gemini: { label: 'Google Gemini', provider: 'gemini', baseUrl: '', modelId: 'gemini-2.5-flash', icon: Cpu },
  deepseek: { label: 'DeepSeek V3', provider: 'external', baseUrl: 'https://api.deepseek.com', modelId: 'deepseek-chat', icon: Zap, apiKey: 'sk-083f9382d0264f1ab3023ca8c1256891' },
...
export const Layout: React.FC<LayoutProps> = ({ children, theme, onToggleTheme, currentView, onNavigate, textSettings, onUpdateTextSettings, onLogout, userAvatar, onUpdateAvatar }) => {
  ...
  // Initialize with DeepSeek as default if no config
  const [selectedPreset, setSelectedPreset] = useState<string>('deepseek');
  const [aiConfig, setAiConfig] = useState<AIConfig>({
    provider: 'external',
    apiKey: 'sk-083f9382d0264f1ab3023ca8c1256891',
  ...
  const handlePresetChange = (presetKey: string) => {
    ...
    setAiConfig(prev => ({
      ...
      apiKey: preset.apiKey || '', // Auto-fill key if available in preset
    ...
    <div className="h-screen w-screen flex bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors duration-300 overflow-hidden relative">
      ...
      <nav className={`w-20 bg-[var(--bg-secondary)] border-r border-[var(--border-color)] flex flex-col items-center py-3 shrink-0 z-50 ${!isOnline ? 'pt-10' : ''}`}>
        ...
        <div className="mb-4 p-1.5 bg-white/10 rounded-xl">
          <img src="/logo-pulpito.png" alt="App Logo" className="w-8 h-8 object-contain" />
        </div>
